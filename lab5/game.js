...
function shootBullet(){
    canShoot = false;
    bulletTimer = setTimeout("canShoot = true",SHOOT_INTERVAL);
    var new_bullet = document.createAttributeNS("http://www.w3.org/2000/svg", "use");
    new_bullet.setAttribute("x",player.position.x + PLAYER_SIZE.w / 2 - BULLET_SIZE.w / 2);
    new_bullet.setAttribute("y",player.position.y + PLAYER_SIZE.h / 2 - BULLET_SIZE.h / 2)
    monster.setAttributeNS("http://www.w3.org/1999/xlink", "xlink:href", "#bullet");
    document.getElementById("bullets").appendChild(new_bullet);
};
...
function collisionDetection() {
    ...
    for (var i = 0; i < monsters.childNodes.length; i++) {
        ...
        var pos = new Point(x, y);
        if(intersect(player.position, PLAYER_SIZE, pos, MONSTER_SIZE)){
            alert("Game over!");
    ...
    for (var i = 0; i < bullets.childNodes.length; i++) {
        ...
        var x1= parseFloat(bullet.getAttribute("x"));
        var y1 = parseFloat(bullet.getAttribute("y"));
        var bpos = new Point(x1,y1);
        ...
        if(intersect(bpos, BULLET_SIZE, mpos, MONSTER_SIZE)){
